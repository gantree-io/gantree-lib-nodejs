...
function check_required_files() {
  ...
  if (!fs.existsSync(GENERIC_CHAINSPEC_PATH)) {
    console.error(
      chalk.red(
        `[Gantree] ${GEN_CHAINSPEC_FILENAME} missing at path: ${GENERIC_CHAINSPEC_PATH}`
      )
    )
    files_missing = true
  }
  if (!fs.existsSync(VALIDATORSPEC_PATH)) {
    console.error(
      chalk.red(
        `[Gantree] ${VALIDATORSPEC_FILENAME} missing at path: ${VALIDATORSPEC_PATH}`
      )
    )
    files_missing = true
  }
...
module.exports = {
  do: async cmd => {
    ...
    for (let i = 0; i < validatorspec.validators.length; i++) {
      ...
      runtime_obj.balances.balances.push([validator_n.sr25519.address, balance])
